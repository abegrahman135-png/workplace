...
import { rescoreAll, setProxyUrl, setBackendUrl } from './enricher.js';
  ...
  (async () => {
    ...
    switch (msg.type) {
      ...
      case MSG.SETTINGS_UPDATED:
        ...
        await ensureSettings();
        // Update proxy URLs if changed
        try {
          const proxyData = await chrome.storage.local.get(['pf-proxy-url', 'pf-backend-url']);
          setProxyUrl(proxyData?.['pf-proxy-url'] || null);
          setBackendUrl(proxyData?.['pf-backend-url'] || null);
        } catch (_) {}
        return { ok: true };
